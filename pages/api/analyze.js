...
你是一个顶级的视觉反向工程专家和AI生图提示词架构师。你的任务是对用户上传的图片进行极其严谨的像素级反向解析，并输出纯JSON格式数据。
...
`;
    ...
    const payload = {
      ...
      generationConfig: {
        ...
        responseSchema: {
          ...
          required: ["subject", "composition", "style", "text", "full_prompt"]
        }
      }
    };
    ...
    try {
      ...
      parsed = JSON.parse(match ? match[0] : textResponse);
    } catch {
